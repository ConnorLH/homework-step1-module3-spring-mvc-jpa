...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private List<HandlerExecutionChain> handlerMapping = new ArrayList<>();
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                // 添加拦截器
                HandlerInterceptor interceptor = (HandlerInterceptor) ioc.get(HandlerInterceptor.class.getName());
...
//        method.invoke() //
        ...
        Handler handler = (Handler)handlerChain.getHandler();

        // 执行拦截器
        List<HandlerInterceptor> interceptorList = handlerChain.getInterceptorList();
        for (HandlerInterceptor handlerInterceptor : interceptorList) {
            boolean result = handlerInterceptor.preHandle(req, resp, handler);
            if(!result){
                return;
            }
        }
    ...
    private HandlerExecutionChain getHandler(HttpServletRequest req) {
        ...
        for (HandlerExecutionChain handlerChain : handlerMapping) {
            Matcher matcher = ((Handler)handlerChain.getHandler()).getPattern().matcher(url);
            if (!matcher.matches()) {
                continue;
            }
            return handlerChain;
